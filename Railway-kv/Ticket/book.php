...
if (array_key_exists("submit", $_POST)) {
    ...
    if ($q == true) {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            echo "<tr><td>{$row['from_station_name']}</td><td>{$row['to_station_name']}</td><td><form action='form.php' method='post'>{$row['train_name']}</td><td>{$row['train_number']}</td><td>{$f}</td><td><input type='hidden' value='{$row['train_number']}' name='tno'><input type='hidden' value='{$f}' name='fare'><input type='hidden' value='{$num}' name='num'><input type='submit' value='BOOK' name='book'></form></td></tr></section>";
        }
